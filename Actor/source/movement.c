...
void Movement_MoveTowardsNextPos(NpcMaker* en, PlayState* playState, float speed, movement_type movementType, bool ignoreY, bool setAnims)
{
    ...
    else
    {
        ...
        if (!en->isMoving)
        {
            ...
            #if LOGGING == 1
                osSyncPrintf("_Started movement.");
            #endif  
        }
        ...
        case MOVEMENT_PATH:
        {
            ...
            if (!en->isMoving)
            {
                ...
                {   
                    ...
                    {
                        #if LOGGING == 1
                            osSyncPrintf("_Error: Next node was NULL.");
                        #endif                         
...
void Movement_SetNextPos(NpcMaker* en, Vec3f* next_pos)
{
    ...
    #if LOGGING == 1
        osSyncPrintf("_Set next position at %08x, %08x, %08x.", en->movementNextPos.x, en->movementNextPos.y, en->movementNextPos.z);
    #endif  
}
...
bool Movement_HasReachedDestination(NpcMaker* en, float distance_margin)
{
    ...
    #if LOGGING == 1
        if (result)
            osSyncPrintf("_Reached end position or travelled far enough.");
    #endif  
